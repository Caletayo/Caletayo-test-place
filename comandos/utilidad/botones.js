...
const botones = new MessageActionRow()
			.addComponents(
				new MessageButton()
					...
					.setLabel('Medir latencia 🎈')//el nombre del boton
...
const filter = i => i.user.id === message.author.id;
  ...
  if (i.customId === 'ping') {
    await i.reply({ content: "**🤖 `|` Bot**: "+ Math.floor(message.createdTimestamp - Date.now()) +"\n**🏐 `|` Api**: " + client.ws.ping,ephemeral: true });
    }
      if (i.customId === 'rojo') {
    await i.reply({ content: '¡Se hizo clic en otro botón! (4)', embeds: [embed2], ephemeral: true });
	}
});
}
}